...
class VectorStore:
    """Векторное хранилище на основе ChromaDB с OpenAI embeddings."""
    ...
    def __init__(self, collection_name: str = "rag_collection", persist_directory: str = "./chroma_db"):
        ...
        try:
            ...
            print(f"Коллекция '{collection_name}' загружена. Документов: {count}")
        except Exception:
            # Удаляем старую коллекцию с несовместимыми embeddings
            try:
                self.client.delete_collection(collection_name)
                print(f"Удалена устаревшая коллекция с несовместимой размерностью")
            except Exception as del_error:
                print(f"Не удалось удалить коллекцию: {del_error}")
        ...
        # OpenAI клиент для создания embeddings (через ProxyAPI)
    ...
    def load_documents(self, file_path: str):
        ...
        Args:
            file_path: путь к файлу или директории с документами
        """
        ...
        if os.path.isfile(file_path):
            ...
            # Директория — рекурсивный поиск .txt файлов
        ...
        if not files_to_process:
            raise ValueError(f"Не найдено .txt файлов по пути {file_path}")
        ...
        for filepath in sorted(files_to_process):
            with open(filepath, 'r', encoding='utf-8') as f:
                content = f.read()
                if content.strip():
                    all_texts.append(f"--- Файл: {os.path.basename(filepath)} ---\n{content}")
    ...
    def _create_embedding(self, text: str) -> List[float]:
        """
        Создание векторного представления текста через OpenAI (ProxyAPI).
